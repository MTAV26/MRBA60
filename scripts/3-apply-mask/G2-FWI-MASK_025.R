# ============================================================
# FWI95d 2003-2024: RELLENO KNN SOBRE TIERRA + PLOTS DE CONTROL
# ============================================================
...
dir_fwi  <- "/mnt/disco6tb/MRBA60/data/A2_TEMP/FWI/"
...
file_fwi <- paste0(dir_fwi, "fwi-era5_count_exceed95_200301-202412_0.25.nc")
...
file_out_with_original <- paste0(
  dir_adj,
  "FWI95d-2003_2024-MONTHLY-025-mask-landsea-KNN-with-original.RData"
)

file_log_csv <- paste0(
  dir_adj,
  "FWI95d-2003_2024-MONTHLY-025-mask-landsea-KNN-fill_log.csv"
)

file_summary_rdata <- paste0(
  dir_adj,
  "FWI95d-2003_2024-MONTHLY-025-mask-landsea-KNN-summary.RData"
)
...
# Como FWI95d es un recuento mensual de días, se recomienda TRUE.
# Si prefieres mantener valores continuos interpolados, usa FALSE.
round_counts <- TRUE

# FUNCIÓN PARA DÍAS DEL MES
source("/mnt/disco6tb/Dropbox/UAH/FireCCI60/paper/review_v1/scripts/0_functions/get_days_in_month.R")
# ============================================================
...
days_month <- sapply(dates_full, get_days_in_month)
...
# ============================================================
# CARGAR FWI95d DESDE NETCDF
# ============================================================

nc <- nc_open(file_fwi)

cat("\nVariables disponibles en FWI95d NetCDF:\n")
print(names(nc$var))

var_fwi <- names(nc$var)[
  grepl("fwi|FWI|count|exceed|95", names(nc$var), ignore.case = TRUE)
][1]

if (is.na(var_fwi)) {
  var_fwi <- names(nc$var)[1]
}

cat("Variable FWI95d usada:", var_fwi, "\n")

FWI95d_tot <- ncvar_get(nc, var_fwi)

nc_close(nc)

cat("Dimensiones FWI95d_tot:\n")
print(dim(FWI95d_tot))

# Guardar copia original antes del relleno KNN
FWI95d_original <- FWI95d_tot

# ============================================================
# COMPROBAR DIMENSIONES DEL FWI95d
# ============================================================

nrows <- dim(FWI95d_tot)[1]
ncols <- dim(FWI95d_tot)[2]
time_common <- dim(FWI95d_tot)[3]

cat("Número de meses esperado:", length(dates_full), "\n")
cat("Número de meses en FWI95d:", time_common, "\n")

if (time_common != length(dates_full)) {
  warning("El número de meses del FWI95d no coincide con 2003-2024.")
}

# ============================================================
# COMPROBAR RANGO ORIGINAL
# ============================================================

cat("\nResumen FWI95d original:\n")
print(summary(as.vector(FWI95d_tot)))

cat("\nRango FWI95d original:\n")
print(range(FWI95d_tot, na.rm = TRUE))
...
if (!all(dim(FWI95d_tot)[1:2] == dim(status_matrix2_tot)[1:2])) {
  stop(
    "Las dimensiones espaciales de FWI95d_tot y FireMask_AF3030F no coinciden.\n",
    "FWI95d_tot: ", paste(dim(FWI95d_tot)[1:2], collapse = " x "), "\n",
    "FireMask: ", paste(dim(status_matrix2_tot)[1:2], collapse = " x ")
  )
}

if (!all(dim(FWI95d_tot)[1:2] == dim(land_sea_mask))) {
  stop(
    "Las dimensiones espaciales de FWI95d_tot y land_sea_mask no coinciden.\n",
    "FWI95d_tot: ", paste(dim(FWI95d_tot)[1:2], collapse = " x "), "\n",
    "land_sea_mask: ", paste(dim(land_sea_mask), collapse = " x ")
  )
}

if (dim(FWI95d_tot)[3] != dim(status_matrix2_tot)[3]) {
  stop(
    "La dimensión temporal de FWI95d_tot y FireMask_AF3030F no coincide.\n",
    "FWI95d_tot time: ", dim(FWI95d_tot)[3], "\n",
...
cat("Píxeles-tiempo con fuego y FWI95d NA sobre océano:", n_target_ocean, "\n")

mask_mes_1 <- status_matrix2_tot[, , 1] == 1 &
  is.na(FWI95d_tot[, , 1]) &
  land_sea_mask == 1

image(mask_mes_1, main = "Píxeles FWI95d a rellenar - mes 1")

# ============================================================
# KNN PARA RELLENAR FWI95d SOLO EN TIERRA
...
#
# Como FWI95d es número de días:
#   - se calcula la media de vecinos
#   - se limita a 0-días del mes
#   - opcionalmente se redondea a entero
# ============================================================
...
for (tt in seq_len(time_common)) {
  ...
  fwi_mes <- FWI95d_tot[, , tt]
  fire_mes <- status_matrix2_tot[, , tt]
  max_days_mes <- days_month[tt]
  
  # Donantes: FWI95d válido y tierra
  valid_xy <- which(
    !is.na(fwi_mes) &
      land_sea_mask == 1,
    arr.ind = TRUE
  ...
  # Objetivo: fuego, FWI95d NA y tierra
  missing_xy <- which(
    fire_mes == 1 &
      is.na(fwi_mes) &
      land_sea_mask == 1,
  ...
  for (i in seq_len(nrow(missing_xy))) {
    ...
    if (length(vals_validos) > 0) {
      ...
      nuevo_valor <- mean(vals_validos)
      
      # Limitar al rango físico posible del mes
      nuevo_valor <- max(0, min(nuevo_valor, max_days_mes))
      
      # Si se quiere conservar como número entero de días
      if (round_counts) {
        nuevo_valor <- floor(nuevo_valor + 0.5)
        nuevo_valor <- max(0, min(nuevo_valor, max_days_mes))
      }
      
      fwi_mes[r0, c0] <- nuevo_valor
  ...
  # Actualizar cubo FWI95d
  FWI95d_tot[, , tt] <- fwi_mes
  ...
  rm(fwi_mes, fire_mes, valid_xy, missing_xy, nn)
  gc()
}
...
# ============================================================
# COMPROBAR RANGO FINAL
# ============================================================

cat("\nResumen FWI95d final:\n")
print(summary(as.vector(FWI95d_tot)))
...
summary_knn <- list(
  variable = "FWI95d",
  description = "Monthly number of days when FWI exceeded the monthly 95th percentile",
  period = "2003-2024",
  k_nn = k_nn,
...
# RData principal: solo lon, lat y FWI95d_tot
save(
  lon,
  lat,
  FWI95d_tot,
...
cat("\nArchivo auxiliar con FWI95d original guardado en:\n")
...
cat("\nResumen FWI95d original:\n")
print(summary(as.vector(FWI95d_original)))

cat("\nResumen FWI95d rellenado:\n")
print(summary(as.vector(FWI95d_tot)))
...
df_fwi_original <- data.frame(
  expand.grid(lon = lon, lat = lat),
  FWI95d = as.vector(FWI95d_original[, , tt_plot])
)

df_fwi_filled <- data.frame(
  expand.grid(lon = lon, lat = lat),
  FWI95d = as.vector(FWI95d_tot[, , tt_plot])
)
...
filled_cells <- is.na(FWI95d_original[, , tt_plot]) &
  !is.na(FWI95d_tot[, , tt_plot]) &
...
fwi_min <- min(
  c(df_fwi_original$FWI95d, df_fwi_filled$FWI95d),
  na.rm = TRUE
)
...
p_original <- ggplot() +
  ...
  geom_raster(
    data = df_fwi_original,
    aes(x = lon, y = lat, fill = FWI95d)
  ) +
...
file_plot_pdf <- paste0(
  dir_plot,
  "FWI95d_original_vs_KNN_SEAsia_",
