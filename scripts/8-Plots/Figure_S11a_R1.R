...
# ==========================================================
# FIGURA SUPLEMENTARIA S11 — Mapas mensuales de área quemada
# MRBA60 / FireCCI60 NetCDF
# Proyección Robinson, sin Antártida
# Salida: PDF + JPEG por mes
# ==========================================================

# ==== rutas ====
base_dir <- "/mnt/disco6tb/MRBA60-2/results/C1-MRBA60-2003-2024-NetCDF/"
out_dir  <- "/mnt/disco6tb/MRBA60-2/results/D1-Plots/FigureS11_R1_Robinson"
...
# ==== recorte para excluir Antártida ====
lat_min_plot <- -60
lat_max_plot <-  90

# ==== colores / clases (km²) ====
cols <- c(
  "#313695", "#3150A7", "#4575B4", "#ABD9E9",
  "#FFFFBF",
  "#FDAE61", "#D73027", "#A50026", "#870000"
)
...
breaks_fixed <- c(0, 1, 5, 10, 25, 50, 100, 200, 400, Inf)
...
# ==========================================================
# Función auxiliar para ordenar archivos mensuales
# ==========================================================
...
# ==========================================================
# Función para convertir matriz [lon, lat] a SpatRaster lon/lat
# ==========================================================
...
# ==========================================================
# Funciones para crear líneas densificadas en lon/lat
# Evita diagonales al proyectar a Robinson
# ==========================================================
...
# Densificados antes de proyectar para que no salgan diagonales
# ==========================================================
...
for (i in seq_along(years)) {
  ...
  for (f in files_y) {
    ...
    ba  <- ncdf4::ncvar_get(nc, "burned_area")
    
    ncdf4::nc_close(nc)
    
    # ------------------------------------------------------
    # Normalizar orientación a [lon, lat]
    # ------------------------------------------------------
    
    if (nrow(ba) == length(lat) && ncol(ba) == length(lon)) {
      ba <- t(ba)
    }
    
    if (!(nrow(ba) == length(lon) && ncol(ba) == length(lat))) {
      stop(
        "Dimensiones no compatibles en archivo: ", f,
        "\nDim ba: ", paste(dim(ba), collapse = " x "),
        "\nlon: ", length(lon),
        "\nlat: ", length(lat)
      )
    }
    ...
    # ------------------------------------------------------
    # Convertir a km² y eliminar ceros
    # ------------------------------------------------------
    
    ba_km2 <- ba / 1e6
    ba_km2[ba_km2 == 0] <- NA
    
    max_ba <- suppressWarnings(max(ba_km2, na.rm = TRUE))
    
    if (!is.finite(max_ba)) {
      message("Archivo sin valores positivos. Se omite: ", bname)
      next
    }
    ...
    # method = near para no interpolar clases/valores
    # ------------------------------------------------------
    ...
    plot_monthly_ba_robinson <- function() {
      ...
      tw   <- max(strwidth(labels, cex = 0.90))
      w1   <- dx + txt_pad + tw
      w2   <- dx + txt_pad + tw
      wTot <- w1 + gap_cols + w2
    ...
    pdf_file <- file.path(out_dir, sprintf("BA_%d-%s_Robinson.pdf", y, m))
    ...
    jpeg_file <- file.path(out_dir, sprintf("BA_%d-%s_Robinson.jpeg", y, m))
